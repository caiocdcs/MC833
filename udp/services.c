...
char* getInfosByEmail(char *email) {
	...
	for(i = 0; i < TAM; i++) {
		...
		if (strcmp(profiles[i].email, email) == 0) {
			info = realloc(info, (strlen(info) + strlen(profiles[i].email) + strlen(profiles[i].name) + strlen(profiles[i].city) + strlen(profiles[i].course) + strlen(profiles[i].skill) + strlen(profiles[i].photoUrl) + strlen(profiles[i].exp) + 11) * sizeof(char));
			strcat(info, profiles[i].email);
			strcat(info, ", ");
			strcat(info, profiles[i].name);
			strcat(info, ", ");
